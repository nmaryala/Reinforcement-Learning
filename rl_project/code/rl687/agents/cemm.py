...
class CEMM(BBOAgent):
    ...
    def __init__(self, theta:np.ndarray, sigma:float, popSize:int, numElite:int, numEpisodes:int, evaluationFunction:Callable, histories, c,  epsilon:float=0.0001):
        #TODO
    ...
    def train(self)->np.ndarray:
        ...
        for k in range(self.popSize):
            ...
            condition = jprime - 2*std/np.sqrt(500)*stats.t.ppf(1-0.05, 499)
            dic[tuple(thetaPrime)] = jprime
            if jbest < jprime:
                # if condition >= self._c/2:
                print(jprime, jbest, self._JBest,  std, condition, self._c)
                jbest = jprime
